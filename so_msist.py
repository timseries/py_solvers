    ...
    def solve(self,dict_in):
        ...
        if self.str_sparse_pen == 'l0rl2': #msist
            ...
            if self.str_sparse_pen == 'vbmm':
                ...
                ary_a = get_gamma_shapes(W * dict_in['x_0'])
        ...
        for n in np.arange(self.int_iterations):
            ...
            for s in xrange(w_n.int_subbands-1,-1,-1):
                ...
                if self.str_sparse_pen == 'l0rl2': #msist
                ...
                elif self.str_solver_variant == 'vbmm': #vbmm    
                    if n == 0:
                        sigma_n = 0
                    else:
                ...
                elif self.str_solver_variant == 'vbmm_hmt': #vbmm    
                    if n == 0:
                        sigma_n = 0
                    else:
                        sigma_n = (1.0 / nu[n]**2 * alpha[s] + S_n.get_subband(s))**(-1)
                    if s < w_n.int_subbands - 6:    
                    ...
                    S_n.set_subband(s, (g_i + 2.0 * ary_a[s]) / 
                                    (nabs(w_n.get_subband(s))**2 + sigma_n + 2.0 * adj_factor * np.abs(s_parent_us)**2))
                else:
